...
export function isMarkdownMetadata(
	metadata: any,
): metadata is MarkdownMetadata {
	return (
		metadata &&
...
export function isMediaMetadata(metadata: any): metadata is MediaMetadata {
	return (
		metadata && "attachments" in metadata && Array.isArray(metadata.attachments)
	);
}
...
// Legacy alias
export function isTextOnlyMetadata(
	metadata: any,
): metadata is MarkdownMetadata {
	return isMarkdownMetadata(metadata);
}

export function isAccountMention(
	mention: PostMention,
): mention is PostMention & { account: string } {
	return "account" in mention;
}

export function isGroupMention(
	mention: PostMention,
): mention is PostMention & { group: string } {
